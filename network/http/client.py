# This file is fork from https://github.com/balloob/micropython-http-client/blob/master/http_client.py
# Origin authos: balloob
# Enhanced: cwyark
import usocket
import ujson
try:
...
class Response(object):
    def __init__(self, status_code, sock, headers):
...
def request(method, url, json=None, timeout=None, headers=None):
    ...
    while ret != b'\r\n':
        ...
        try:
            lret = ret.split()
            ret_headers[lret[0]] = lret[1]
        except:
            pass

    return Response(int(status), sock, ret_headers)
